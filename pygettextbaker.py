...
def create_init_pyfile(f, generate_for):
    def write(msg):
        f.write(msg + "\n")
    write("# -*- coding: utf-8 -*-")
    write("# Autogenerated by " + APPNAME + ", " + VERSION)
    
    domains, langs = generate_for
    write("")
    write("LANGS = (" + ", ".join(repr(lang) for lang in langs) + ",)")
    write("DOMAINS = (" + ", "
        .join(repr(domain) for domain in domains) + ",)")
    write("")
    # clone functions
    write("import os")
    write("import io")
...
def translation(domain, localedir=None, languages=None,
    ...
    for mofile in mofiles:
        ...
        if t is None:
            data = None
            lang, domain = mofile
            if lang in LANGS and domain in DOMAINS:""")
...
def baker(source, destination):
    ...
    with open(os.path.join(destination, "__init__.py"), "w",
        ...
        create_init_pyfile(f, generate_for = (alldomains, langs.keys()))
